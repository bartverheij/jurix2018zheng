...
\normalsize
In order to distinguish the propositions in the case model easily, each elementary proposition in this model has been given an abbreviation. These abbreviations are shown in table 1.

\footnotesize
\begin{longtable}{p{3mm}p{100mm}}
\caption{Elementary propositions in the case model of copyright infringement\label{tab:long}}\\
%\begin{table}[htbp]
...
%\begin{enumerate}
%\item If the defendant has following actions:
%\begin{itemize}
...
%\item If the defendant was regarded as violating someone else's copyright for the purpose of making profits, then the defendant will be sentenced to the crime of copyright infringement. %($ifg \wedge fpp \Rightarrow crc$).

%\item If \textit{the amount of defendant's illegal gains was large or existed other serious circumstances}, the defendant shall be sentenced to 3 kinds of punishments, \textit{fixed-term imprisonment of not more than 3 years and fined}; \textit{criminal detention and fined} and only \textit{fined}. %($crc \wedge ils \leadsto l3fti \wedge fin$)($crc \wedge ils \leadsto cdt \wedge fin$)($crc \wedge ils \leadsto fin$).

%\item If \textit{the amount of defendant's illegal gains was huge or existed other especially serious circumstances}, then the defendant shall be sentenced to \textit{fixed-term imprisonment of not less that three years but not more than seven years and fined}. %(this can be represented as $crc \wedge ihe \Rightarrow m3fti \wedge fin$.)

%\item There are two kinds of punishments which are possible to be put on probation, if the defendant satisfied the conditions of probation: 
%\begin{itemize}
%\item fixed-term imprisonment of not more than 3 years and fined; %($crc \wedge l3fti \wedge fin \wedge cpb \Rightarrow pbt$); 
%\item criminal detention and fined. %($crc \wedge cdt \wedge fin \wedge cpb \Rightarrow pbt$).
%\end{itemize}
%\end{enumerate}
...
In the light of Art. 217 and the judicial explanations related to it, a case model can be built. The model has 46 cases. Case 1 is built by the principle of "presumption of innocence". Case 2 is for the first defeating circumstance mentioned above. Case 3 shows the scenario that the defendant violated someone else's copyright, but he didn't do it for making profits. From Case 3 to Case 13, different punishments for the defendant's action in Art. 217:1 are listed. In the same way, different punishments for the defendant in Art. 217:2, Art. 217:3 and Art. 217:4 will also be listed into the case model. Thus Case 14 to Case 46 have similar components with Case 4 to Case 13, except the acts of infringement on copyright are different. In this model, Case 1 is the most preferred case, as we consider the "presumption of innocence" is the most important principle in the process of decision making. Besides, in some situations, the defendant will not be regarded as violating someone else's copyright, for instance, Case 2 and Case 3. We put them in second place. The rest of cases in the preferred relation are the specific punishments of copyright infringement, they are less preferred than the cases in first and second place.

The Chinese copyright infringement model is represented as {\mf model\_num(2)} in the program. The following codes show the first 13 cases in the model and their preferred relation. From this copyright infringement model, we can get the argument diagram illustrated in Figure 2 (left). This argument has multi-steps, and it is corresponding to the model we built.
\begin{lstlisting}[caption={Definition of the Chinese copyright infringement case model in Prolog},captionpos=b,float]
...
According to the definitions of Verheij's case model formalism, the rule $ifg \wedge fpp \Rightarrow crc$ is valid in this model. As the model shows, this argument is coherent, as all cases imply sentence $ifg \wedge fpp \wedge crc$ which is the case made by the argument $(ifg \wedge fpp, crc)$. Besides, all the cases which imply the premises $ifg \wedge fpp$, also imply the conclusion $crc$. So argument $(ifg \wedge fpp, crc)$ is conclusive in the case model. And these arguments are also conclusive in the model:

\begin{itemize}
\item $(C,  \geq) \models crc \wedge l3fti \wedge fin \wedge cpb \Rightarrow pbt$
\item $(C,  \geq) \models crc \wedge cdt \wedge fin \wedge cpb \Rightarrow pbt$
\end{itemize}

Argument $(pac \wedge ite \wedge pco, ifg)$ is presumptively valid. In the light of the copyright infringement model, this argument is coherent as Case 2 has implied the case made by the argument $(pac \wedge ite \wedge pco, ifg)$. And Case 2 is also the strongest case among the cases which implied the premises of this argument. So we say the argument $(pac \wedge ite \wedge pco, ifg)$ is presumptively valid in the case model. As the argument diagram in Figure 2 shows, the defeating circumstance $pec$ successfully attacks the argument $(pac \wedge ite \wedge pco, ifg)$, it makes the argument $(pac \wedge ite \wedge pco \wedge pec, \neg ifg)$ presumptively valid, which means this circumstance is rebutting. In the same way, the following arguments are considered to be validate.

\begin{itemize}
\item $(C,  \subseteq) \models crc \leadsto ils$
...
The Prolog program we designed has confirmed the validity of the arguments above. The Prolog clauses in Figure 2 (right) are considered to be true in the Prolog queries, which means the results of Prolog program are corresponded to the analysis about Chinese copyright infringement model above.

\section{Discussion and Conclusion}

\noindent A case model in the background of Chinese criminal law has been built as a case study for the program. As the arguments implied in this case model are corresponded to the rules in statutes, Verheij's theory has been proved that it is suitable for Chinese legal system. %The preferred relation established by Verheij solves the priority relationship among the cases in case model, so that the presumptive arguments in legal reasoning can be concluded by this theory. In other words, some legal issues can be solved, such as the rules used for legal reasoning, which are beyond the statutes, for instance, "presumption of innocence" in the background of Chinese legal system. Although it is not in any statute, every judge will think about it during the process of making decisions. So we can place this rule as several cases in the preferred relation of a case model properly to solve this issue. Therefore, we believe arguments embedded values are more suitable for dealing with practical issues in legal reasoning. 

The program we introduced in this paper intends to make the modeling process with less mistakes and its main function is automatically verifying the validity of the arguments implicated in a case model and the case model itself. This program has been proved that it is completely suitable for those models based on Verheij's theory, even these models have different backgrounds. Compare with the original hand-made modeling way, it can reduce mistakes during the process of modeling significantly, as this program can verify the model automatically. This is a successful attempt as a computational implementation for Verheij's case model formalism which can make Verheij's theory more useful in practical environment. 
...
The results of this paper shows that Verheij's case model formalism can be used to model the object with complex argument structure, and the program developed in this paper is feasible for the models based on Verheij's theory. %It also proves that the Prolog program we built can well combine cases, rules and arguments, which is not only applicable to the civil law system but also to the Chinese legal system. 
AI and legal reasoning technology needs to combine rule-based reasoning, case-based reasoning and argumentation together, and argumentation technology can be the bridge of both cases and rules.

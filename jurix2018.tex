...
Presumptive validity of an argument is checked by first determining a case that makes the case coherent (i.e., finding a case in which both the premises and the conclusions of the argument hold) and then checking for each case in which the argument's premises hold whether such a case is of equal or lower ordering. For instance, {\mf argument([vrt],[unl]))} is presumptively valid. However the coherent argument {\mf argument([vrt],[not(unl)]))} is not presumptively valid since cases that show the coherence of the argument (such as case {\mf 114}) are not maximal among the cases in which the premises hold (e.g., case {\mf 105} is higher in the ordering).
